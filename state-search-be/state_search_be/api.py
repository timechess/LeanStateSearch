...
class LeanStateSearchServicer(LeanStateSearchServiceServicer):
    def __init__(self, db: Prisma, theorems):
        ...
        for rev in revs:
            ...
            with open(os.path.join(embedding_path, "index2uuid.json"), "w") as f:
                json.dump(index2uuid, f)
    ...
    async def SearchTheorem(self, request, context):
        ...
        with open(os.path.join(self.embedding_path, rev, "index2uuid.json"), "r") as f:
            index2uuid = json.load(f)
        query = query.split("⊢")
        context = query[0]
        if len(query) == 2:
            goal = query[1]
